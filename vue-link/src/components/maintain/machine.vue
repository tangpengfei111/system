    ...
    <div class="loading dialog-box" v-if="childPageIsShow">
      <div class="dialog">
        <div class="content-item">
          <div>设备名</div>
          <input v-model="params.name" placeholder="请填写设备名称">
        </div>
        <div class="content-item">
          <div>设备编号</div>
          <input v-model="params.no" placeholder="请填写设备编号">
        </div>
        <div class="footer">
...
export default {
  ...
  data() {
    return {
      tableHeader: [              // 表格头部信息
        { label: '设备编号', prop: 'no', editor: false },
        { label: '设备名称', prop: 'name', editor: false },
  ...
  methods: {
    ...
    getAllMachines(text) {
      ...
      this.$http.post('/machineController/pageList',params).then(res => {
