<template>
  <div class="maintain-child">
    <div class="header">
      <div class="title">
        <div>{{this.$route.meta.til || '客户管理'}}</div>
      </div>
      <div class="func-bar">
    ...
    <div class="loading dialog-box" v-if="childPageIsShow">
      <div class="dialog">
        <div class="content-item">
          <div>客户名</div>
          <input v-model="params.name" placeholder="请填写客户名称">
        </div>
        <div class="content-item">
          <div>客户编号</div>
          <input v-model="params.no" placeholder="请填写客户编号">
        </div>
        <div class="footer">
...
export default {
  ...
  data() {
    return {
      tableHeader: [              // 表格头部信息
        { label: '客户编号', prop: 'no', editor: false },
        { label: '客户名称', prop: 'name', editor: false },
  ...
  methods: {
    ...
    getAllCustomers(currentPage,text) {
      ...
      this.$http.post('/customerController/pageList',params).then(res => {
